                                    ...
                                    <div class="addNewSub" v-for="(item, index) in data.items" :key="index">
                                        ...
                                        <div class="addNewFirst adnright" v-if="item.name == 'Phone' && item.status == true">
                                            ...
                                            <div class="addNewFirst adnright" v-if="item.name == 'Department/Division' && item.status == true">
                                                ...
                                                <select class="addNewInput" :class="{ filderror: v3$.dept.$error }" v-model="state3.dept"  v-on:change="getDesignationByDepartmentIdWiseList(state3.dept)">
                                            ...
                                            <div class="addNewFirst adnright" v-if="item.name == 'Department/Division' && item.status == true">
                                                ...
                                                <select class="addNewInput" :class="{ filderror: v3$.dept.$error }" v-model="state3.designation">
                                            ...
                                            <div class="addNewFirst adnleft" v-if="item.name == 'Supervisor/Manager Name' && item.status == true">
                                                ...
                                                <select v-model="state3.reporting_manager">
                                                    ...
                                                    <option v-for="rManager in reportingManagerList" :key="rManager._id" :value="rManager._id">{{ rManager.name }}</option>
                                                </select>
                                            </div>
                                            ...
                                            <div class="addNewFirst adnright" v-if="item.name == 'Benifit Related Documents' && item.status == true">
                                                ...
                                                <input type="file" ref="benifitRelatedImage" name="benifitRelatedImage" class="addNewInput" @change="updateBenifitDoc">
                                            </div>
                                            ...
                                            <div class="addNewFirst adnleft" v-if="item.name == 'Highest Education Level' && item.status == true">
                                                ...
                                                <select v-model="state3.edu_level">
                                                    <option value="undefined" disabled>-Select One-</option>
                                                    <option v-for="education in educationList" :key="education._id" :value="education._id">{{ education.name }}</option>
                                            ...
                                            <div class="addNewFirst adnright" v-if="item.name == 'Degree/Certificate Obtained' && item.status == true">
                                                ...
                                                <input type="file" ref="degreeObtainedImage" name="degreeObtainedImage" class="addNewInput" @change="updateEducationalDoc">
                                            </div>
                                            ...
                                            <div class="addNewFirst adnright" v-if="item.name == 'Upload Certificates' && item.status == true">
                                                ...
                                                <input type="file" ref="certificateImage" name="certificateImage" class="addNewInput" @change="updateJobExperience">
                                            </div>
                                        ...
                                        <div class="addNewFirst adnright" v-if="item.name == 'Resume/CV' && item.status == true">
                                            ...
                                            <input type="file" ref="CVImage" name="CVImage" class="addNewInput" @change="updateCV">
                                        </div>
                                        <div class="addNewFirst adnright" v-if="item.name == 'Identification Documents' && item.status == true">
                                            <label>Upload ID Proof:</label>
                                            <input type="file" ref="IDProofImage" name="IDProofImage" class="addNewInput" @change="updateID">
                                        </div>
                                    </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            async getEducationManagerList() {
                const formData = {
                    "company_id":this.storage.companyDetails._id
                };
                await axios.post(BASE_URL + '/education-list',formData)
                .then((response)=>{
                    this.educationList =  response.data.educationList;
                    console.log("EDUCATION LIST:: ",response.data);
                })
            },
            async getReportingManagerList() {
                const formData = {
                    "company_id":this.storage.companyDetails._id
                };
                await axios.post(BASE_URL + '/reporting-manager-list',formData)
                .then((response)=>{
                    this.reportingManagerList =  response.data.reportingManagerList;
                    console.log("REPORTING MANAGER LIST:: ",response.data);
                })
            },
            ...
            updateBenifitDoc(e){
                ...
                this.state3.benifitRelatedImage = file;
                this.benifitRelatedImagePath = URL.createObjectURL(file);
                console.log("Benifit Related Image: ", this.state3.benifitRelatedImage);
            },

            updateEducationalDoc(e){
                const file = e.target.files[0];
                this.state3.degreeObtainedImage = file;
                this.degreeObtainedImagePath = URL.createObjectURL(file);
                console.log("Degree Obtained Image: ", this.state3.degreeObtainedImage);
            },

            updateJobExperience(e){
                const file = e.target.files[0];
                this.state3.certificateImage = file;
                this.certificateImagePath = URL.createObjectURL(file);
                console.log("Certificate Image: ", this.state3.certificateImage);
            },

            updateCV(e){
                const file = e.target.files[0];
                this.state3.CVImage = file;
                this.CVImagePath = URL.createObjectURL(file);
                console.log("CV Image: ", this.state3.CVImage);
            },

            updateID(e){
                const file = e.target.files[0];
                this.state3.IDProofImage = file;
                this.IDProofImagePath = URL.createObjectURL(file);
                console.log("ID Proof Image: ", this.state3.IDProofImage);
            },
            ...
            async submitForm(){
                ...
                if (!this.v3$.$error) {
                    ...
                    if (this.state3.dob != '') {
                        ...
                        var formatted_dob = (dob_year.toString()).concat("-",dob_month_string).concat("-",dob_date_string);
                    ...
                    if (this.state3.doj != '') {
                        ...
                        var formatted_doj = (doj_year.toString()).concat("-",doj_month_string).concat("-",doj_date_string);
                    ...
                    formData.append('benifitdoc', this.state3.benifitRelatedImage);
                    formData.append('edu_level', this.state3.edu_level);
                    formData.append('edu_institue', this.state3.institution_name);
                    formData.append('edu_certificate', this.state3.degreeObtainedImage);
                    formData.append('prev_employer', this.state3.prev_emp);
                    formData.append('emp_skills', this.selectedTags);
                    formData.append('experience_certificate', this.state3.certificateImage);
                    formData.append('jobexperience', this.state3.job_exp_years);
                    formData.append('emp_cv', this.state3.CVImage);
                    formData.append('emp_id_doc', this.state3.IDProofImage);
                    ...
                    await axios.post(BASE_URL + '/addempdetails',formData)
                        .then((response)=>{
                            console.log(response);
                            this.tostMessage(response.data.success, response.data.message);

                            window.location.href = "dashboard/"+this.storage.company_id;
                        })
                } else {
                    this.buttonclick = false;
